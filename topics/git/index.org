...
Now edit =setup.py= to reflect new version (remove "=-dev=") and commit:
...
If there is any additional release munging needed, do it now.  If =develop= was essentially ready to release already then there is nothing more to do.  
